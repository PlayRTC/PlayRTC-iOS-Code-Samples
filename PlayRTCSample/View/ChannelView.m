...
// 채널 팝업뷰를 animation 효과를 주어 화면에 표시한다.
...
- (void)drawRect:(CGRect)rect
{
    ...
    // 탭버튼 과 화면 입력 부 와 채널이이디/채널목록 표시부 구분선
...
- (void) initViewLayout
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 1. 화면 상단 채널 탭 버튼 영역
    // USER-A는 먼저 채널을 생성(채널 생성 탭)해야 하고 USER-B는 생성된 채널(채널 입장 탭)의 아이디로 입장해야한다.
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // 1.3 창닫기 버튼
    popCloseBtn = [[ExButton alloc] initWithFrame:CGRectMake(_frame.size.width - (BTN_CLOSE_WIDTH +20), tabButtonPosY - 2.0f, BTN_CLOSE_WIDTH, BTN_CLOSE_HEIGHT)];
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 2. 탭 Contents 영역
    // 채널을 생성 Pannel과 채널 입장 Pannel을 생성
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    
    // 2.1 채널 생성 탭 화면 영역
    tabCreateInPanel = [[UIView alloc] initWithFrame:CGRectMake(10, tabPannelPosY, _frame.size.width - 20.0f, 90.0f)];
    ...
    // 생성된 채널 아디디 표시 부
    CGFloat pHeight = _frame.size.height - tabConnectPanelY - 10.0f;
    tabCreatePanel = [[UIView alloc] initWithFrame:CGRectMake(10, tabConnectPanelY, _frame.size.width - 20.0f, pHeight)];
    ...
    // 2.2 채널 입장 탭 화면 영역
    tabConnectInPanel = [[UIView alloc] initWithFrame:CGRectMake(10, tabPannelPosY, _frame.size.width - 20.0f, 90.0f)];
    tabConnectInPanel.backgroundColor = [UIColor clearColor];
    tabConnectInPanel.hidden = TRUE;
    
    // 2.2.1 사용자 아이디 입력 부 Line
    // 라벨
    ...
    // 채널 목록 조회 버튼
    btnChList = [[ExButton alloc] initWithFrame:CGRectMake(btnPosX, 47.0f, btnWidth, labelHeight)];
    [btnChList addTarget:self action:@selector(btnClick:event:) forControlEvents:UIControlEventTouchUpInside];
    btnChList.tag = 7;
    [btnChList setTitle:@"채널조회" forState:UIControlStateNormal];
    [btnChList setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [tabConnectInPanel addSubview:btnChList];

    [self addSubview:tabConnectInPanel];
    
    // 채널 목록 표시 뷰
    tabConnectPanel = [[UIView alloc] initWithFrame:CGRectMake(10, tabConnectPanelY, _frame.size.width- 20.0f, pHeight)];
...
- (void)btnClick:(id)sender event:(UIEvent *)event
{
    ...
    else if(tag == 5) //채널 생성하기 버튼
    {
        ...
        if(channelName.length == 0) {
            channelName = [NSString stringWithFormat:@"%@님의 채널입니다.", userId];
        }
